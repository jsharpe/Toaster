...
{
public:
  static double raw2Phys(unsigned short rawVal, double deltaMinMax, double min)
  {
    double physVal = ((rawVal * deltaMinMax) / 0x3FFF) + min;
    return physVal;
  }

  static bool raw2Bool(unsigned short rawVal)
  {
    return rawVal != 0;
  }

  static unsigned short phys2Raw(double physVal, double deltaMinMax, double min, unsigned short maxRawVal = 0x3FFF)
  {
    unsigned short rawVal = ((physVal - min) * maxRawVal) / deltaMinMax;
    return rawVal;
  }

  static unsigned short bool2Raw(bool val)
  {
    return val ? 1 : 0;
  }
  ...
  static unsigned int extractRawVal(unsigned char b0, unsigned char b1, unsigned char b2, unsigned char b3, unsigned char b4)
  {
  ...
  static QString extractString(const ByteArray& byteArray)
  {
    QString strVal;
    for(int i = 0; (i < byteArray.size() && byteArray.at(i) != 0); ++i)
  ...
  static ByteArray packString(const QString& strVal)
  {
    ByteArray val;
    for(int i = 0; i < strVal.length(); ++i)
      val.push_back(strVal.at(i).toLatin1());

    val.push_back((char)0);
    return val;
  }

  static QString raw2Octave(unsigned short rawVal)
  {
    if(rawVal > 12)
  ...
  static unsigned short crc16(const ByteArray& blob)
  {
    static const unsigned short sCRCTable[] =
    {
    ...
    const unsigned char* buf = blob.data();
    int size = blob.size();
    ...
    while (size--)
    {
      tmp = *buf++ ^ crc;
      crc >>= 8;
      crc  ^= sCRCTable[tmp & 0xff];
    }

    return crc;
  }

  static unsigned short crc14(const ByteArray& blob)
  {
    return crc16(blob) & 0x3fff;
  }
};
