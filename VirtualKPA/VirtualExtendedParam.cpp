...
bool VirtualExtendedParam::load(const ByteArray &buf) {
  ...
  if (bufSize >= 15) {
    ...
    int valStartIdx = 12;
    while (noOfParams) {
      unsigned int val = Utils::extractRawVal(
          ByteArray(buf.begin() + valStartIdx, buf.begin() + valStartIdx + 5));
      valStartIdx += 5;
...
ByteArray VirtualExtendedParam::createResponse(const ByteArray &,
                                               bool includeLen) {
