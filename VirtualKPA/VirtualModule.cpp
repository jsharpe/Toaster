...
VirtualModule::VirtualModule(AddressPage ap) : mAP(ap) {}
...
VirtualParam *VirtualModule::createParam(const ByteArray &buf) {
  ...
  if (buf.size() >= 10) {
    ...
    case 4:
      ...
      break;
    case 6:
      newParam = new VirtualExtendedParam(
          Utils::extractRawVal(ByteArray(buf.begin() + 7, buf.begin() + 7 + 5)),
          this);
      break;
    default:
...
ByteArray VirtualModule::midiIn(const ByteArray &msg) {
  if (msg.size() >= 10) {
    ...
    if (paramType == 0x06 || paramType == 0x46)
      paramId =
          Utils::extractRawVal(ByteArray(msg.begin() + 8, msg.begin() + 8 + 5));
    else
      paramId = msg[9];
