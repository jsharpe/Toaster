...
QToasterStompEditLCD::QToasterStompEditLCD(QWidget *parent)
    : QToasterLCD(parent), ui(new Ui::QToasterStompEditLCD) {
  ui->setupUi(this);
  setFonts();
  createSkin();
...
void QToasterStompEditLCD::setStompFXType(StompInstance stompInstance,
                                          FXType fxType) {
...
void QToasterStompEditLCD::setStompEnabled(StompInstance stompInstance,
                                           bool enabled) {
  switch (stompInstance) {
  case StompA:
...
void QToasterStompEditLCD::updatePageInfo() {
  if (mMaxPage != Page1) {
    QString text = QString("[Page %1/%2]")
                       .arg((int)mCurrentPage + 1)
                       .arg((int)mMaxPage + 1);
