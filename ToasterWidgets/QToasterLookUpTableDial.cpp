...
QToasterLookUpTableDial::QToasterLookUpTableDial(QWidget *parent)
    : QWidget(parent), mCurrKnobFrameNo(0), mCurrLEDRingFrameNo(0),
      mKnobSize(Big), mLEDRingType(Uni), mMouseY(0), mMinValueIndex(0),
      mMaxValueIndex(0), mMinValue(0), mMaxValue(0), mCurrValue(0),
      mCurrValueText("<0.0>"), mUnit(""), mIsActive(true), mAcceleration(1),
      mLookUpTable(nullptr), mKnobSkinPixmaps(&sBigKnobSkinPixmaps),
      mLEDRingSkinPixmaps(&sUniLEDRingSkinPixmaps) {
  createKnobSkin();
  createLEDRingSkin();

  connect(&mAccelerationTimer, &QTimer::timeout, [=]() { mAcceleration = 1; });
}
  ...
  if (sUniLEDRingSkinPixmaps.isEmpty()) {
    ...
    if (!masterPixmap.isNull()) {
      ...
      for (int i = 0; i < sLEDRingSkinNoOfFrames; i++) {
        x = i * width;
        sUniLEDRingSkinPixmaps.insert(i,
                                      masterPixmap.copy(x, y, width, height));
      }
    }
  }
...
void QToasterLookUpTableDial::setLookUpTable(
    QVector<QPair<int, QString>> *lookUpTable) {
...
void QToasterLookUpTableDial::update(int deltaSteps) {
  if (mLookUpTable != nullptr) {
    ...
    if (newKnobFrameNo < 0)
      mCurrKnobFrameNo =
          sKnobSkinNoOfFrames + (newKnobFrameNo % sKnobSkinNoOfFrames);
...
void QToasterLookUpTableDial::setUnit(QString unit) { mUnit = unit; }
