...
QToasterButton::QToasterButton(QWidget *parent)
    : QWidget(parent), mSkinPixmaps(&sSmallButtonSkinPixmaps), mType(Small),
      mGlobalState(Off), mOnOffState(Off), mClickEmited(false),
      mAnimLabel(this), mpCtxMenuProvider(NULL) {
  ...
  if (sSmallButtonSkinPixmaps.isEmpty()) {
    ...
    if (!masterPixmap.isNull()) {
      ...
      for (int i = 0; i < sSkinNoOfFrames; i++) {
        x = i * width;
        sSmallButtonSkinPixmaps.insert(i,
...
void QToasterButton::resetToOnOffState() { mGlobalState = mOnOffState; }
