...
QToasterEnumDial::QToasterEnumDial(QWidget *parent)
    : QWidget(parent), mKnobSkinPixmaps(&sBigKnobSkinPixmaps),
      mLEDRingSkinPixmaps(&sUniLEDRingSkinPixmaps), mCurrKnobFrameNo(0),
      mCurrLEDRingFrameNo(0), mKnobSize(Big), mLEDRingType(None), mMouseY(0),
      mCurrValueIndex(0), mCurrValueText(""), mIsActive(true) {
  createKnobSkin();
  createLEDRingSkin();
}
...
void QToasterEnumDial::updateValueText() {
  mCurrValueText = "";
  if (mValues.size() > 0 && mCurrValueIndex >= 0 &&
      mCurrValueIndex < mValues.size())
