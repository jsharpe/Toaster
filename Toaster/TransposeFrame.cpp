...
TransposeFrame::TransposeFrame(QWidget *parent) : QWidget(parent) {
  ui.setupUi(this);
}
...
void TransposeFrame::activate(QObject &stomp) {
  ...
  if (mpStomp != nullptr) {
    connect(mpStomp, &Stomp::voice1PitchReceived, this,
            &TransposeFrame::onPitch);
    connect(mpStomp, &Stomp::smoothChordsReceived, this,
            &TransposeFrame::onSmoothChords);
...
void TransposeFrame::displayStompType(StompInstance stompInstance,
                                      FXType fxType) {
  ui.lcdDisplay->setStompFXType(stompInstance, fxType);
}

void TransposeFrame::displayStompEnabled(StompInstance stompInstance,
                                         bool enabled) {
