...
DriveAndToneDSFrame::DriveAndToneDSFrame(QWidget *parent) : QWidget(parent) {
  ui.setupUi(this);
  ...
  if (mpStomp != nullptr) {
    ...
    if (Settings::get().getKPAOSVersion() >= 0x04000000) {
      if (mpStomp->getFXType() == BitShaper) {
        ...
        connect(mpStomp, &Stomp::mixReceived, this,
                &DriveAndToneDSFrame::onMix);
        mpStomp->requestMix();
      }
    }
...
void DriveAndToneDSFrame::displayStompType(StompInstance stompInstance,
                                           FXType fxType) {
  ui.lcdDisplay->setStompFXType(stompInstance, fxType);
}

void DriveAndToneDSFrame::displayStompEnabled(StompInstance stompInstance,
                                              bool enabled) {
