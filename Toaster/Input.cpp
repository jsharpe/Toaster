...
void Input::applyNoiseGate(double noiseGate) {
  midiApplyNoiseGate(Utils::phys2Raw(noiseGate, 10, 0));
}

void Input::applyCleanSense(double cleanSense) {
  midiApplyCleanSense(Utils::phys2Raw(cleanSense, 24, -12));
}

void Input::applyDistortionSense(double distortionSense) {
  midiApplyDistortionSense(Utils::phys2Raw(distortionSense, 24, -12));
}

// InputMidi callbacks
void Input::midiNoiseGateReceived(unsigned short rawVal) {
  emit noiseGateReceived(Utils::raw2Phys(rawVal, 10, 0));
}

void Input::midiCleanSenseReceived(unsigned short rawVal) {
  emit cleanSenseReceived(Utils::raw2Phys(rawVal, 24, -12));
}

void Input::midiDistortionSenseReceived(unsigned short rawVal) {
  emit distortionSenseReceived(Utils::raw2Phys(rawVal, 24, -12));
}
