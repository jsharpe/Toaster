...
HyperChorusFrame::HyperChorusFrame(QWidget *parent) : QWidget(parent) {
  ui.setupUi(this);
...
void HyperChorusFrame::activate(QObject &stomp) {
  ...
  if (mpStomp != nullptr) {
    connect(mpStomp, &Stomp::modulationDepthReceived, this,
            &HyperChorusFrame::onDepth);
    connect(mpStomp, &Stomp::modulationHyperChorusAmountReceived, this,
            &HyperChorusFrame::onAmount);
    connect(
        mpStomp,
        static_cast<void (Stomp::*)(int)>(&Stomp::modulationCrossoverReceived),
        this, &HyperChorusFrame::onCrossover);
...
void HyperChorusFrame::deactivate() {
  if (mpStomp != nullptr) {
    disconnect(mpStomp, &Stomp::modulationDepthReceived, this,
               &HyperChorusFrame::onDepth);
    disconnect(mpStomp, &Stomp::modulationHyperChorusAmountReceived, this,
               &HyperChorusFrame::onAmount);
    disconnect(
        mpStomp,
        static_cast<void (Stomp::*)(int)>(&Stomp::modulationCrossoverReceived),
...
void HyperChorusFrame::displayStompType(StompInstance stompInstance,
                                        FXType fxType) {
  ui.lcdDisplay->setStompFXType(stompInstance, fxType);
}

void HyperChorusFrame::displayStompEnabled(StompInstance stompInstance,
                                           bool enabled) {
