...
MasterVolume::MasterVolume(QObject *parent)
    : QObject(parent), mMainOutputLink(Settings::get().getMainOutputLink()),
      ...
      mMainOutputVolume(0), mMonitorOutputVolume(0), mDirectOutputVolume(0),
      mHeadphoneOutputVolume(0), mSPDIFOutputVolume(0),
...
void MasterVolume::init() {
  connect(&globalObj, &Global::mainOutputVolumeReceived, this,
          &MasterVolume::onMainOutputVolume);
  connect(&globalObj, &Global::monitorOutputVolumeReceived, this,
          &MasterVolume::onMonitorOutputVolume);
  connect(&globalObj, &Global::directOutputVolumeReceived, this,
          &MasterVolume::onDirectOutputVolume);
  connect(&globalObj, &Global::headphoneOutputVolumeReceived, this,
          &MasterVolume::onHeadPhoneOutputVolume);
  connect(&globalObj, &Global::spdifOutputVolumeReceived, this,
          &MasterVolume::onSPDIFOutputVolume);
...
void MasterVolume::dispose() {
  ...
  disconnect(&globalObj, &Global::mainOutputVolumeReceived, this,
             &MasterVolume::onMainOutputVolume);
  disconnect(&globalObj, &Global::monitorOutputVolumeReceived, this,
             &MasterVolume::onMonitorOutputVolume);
  disconnect(&globalObj, &Global::directOutputVolumeReceived, this,
             &MasterVolume::onDirectOutputVolume);
  disconnect(&globalObj, &Global::headphoneOutputVolumeReceived, this,
             &MasterVolume::onHeadPhoneOutputVolume);
  disconnect(&globalObj, &Global::spdifOutputVolumeReceived, this,
             &MasterVolume::onSPDIFOutputVolume);
}
...
void MasterVolume::onMasterVolume(int value) {
  ...
  if (mMainOutputLink) {
    mMainOutputVolume =
        mMainOutputVolume + delta > 0x3FFF ? 0x3FFF : mMainOutputVolume + delta;
    globalObj.applyMainOutputVolume(mMainOutputVolume > 0 ? mMainOutputVolume
                                                          : 0);
  ...
  if (mMonitorOutputLink) {
    mMonitorOutputVolume = mMonitorOutputVolume + delta > 0x3FFF
                               ? 0x3FFF
                               : mMonitorOutputVolume + delta;
    globalObj.applyMonitorOutputVolume(
        mMonitorOutputVolume > 0 ? mMonitorOutputVolume : 0);
    if (mMonitorOutputVolume == 0x3FFF)
      mMasterVolume = 0x3FFF;

    emit monitorOutputVolumeChanged(
        mMonitorOutputVolume > 0 ? mMonitorOutputVolume : 0);
  }

  if (mDirectOutputLink) {
    mDirectOutputVolume = mDirectOutputVolume + delta > 0x3FFF
                              ? 0x3FFF
                              : mDirectOutputVolume + delta;
    globalObj.applyDirectOutputVolume(
        mDirectOutputVolume > 0 ? mDirectOutputVolume : 0);
    if (mDirectOutputVolume == 0x3FFF)
      mMasterVolume = 0x3FFF;

    emit directOutputVolumeChanged(mDirectOutputVolume > 0 ? mDirectOutputVolume
                                                           : 0);
  }

  if (mHeadphoneOutputLink) {
    mHeadphoneOutputVolume = mHeadphoneOutputVolume + delta > 0x3FFF
                                 ? 0x3FFF
                                 : mHeadphoneOutputVolume + delta;
    globalObj.applyHeadphoneOutputVolume(
        mHeadphoneOutputVolume > 0 ? mHeadphoneOutputVolume : 0);
    if (mHeadphoneOutputVolume == 0x3FFF)
      mMasterVolume = 0x3FFF;

    emit headphoneOutputVolumeChanged(
        mHeadphoneOutputVolume > 0 ? mHeadphoneOutputVolume : 0);
  }

  if (mSPDIFOutputLink) {
    mSPDIFOutputVolume = mSPDIFOutputVolume + delta > 0x3FFF
                             ? 0x3FFF
                             : mSPDIFOutputVolume + delta;
    globalObj.applySPDIFOutputVolume(mSPDIFOutputVolume > 0 ? mSPDIFOutputVolume
                                                            : 0);
    if (mSPDIFOutputVolume == 0x3FFF)
      mMasterVolume = 0x3FFF;

    emit spdifOutputVolumeChanged(mSPDIFOutputVolume > 0 ? mSPDIFOutputVolume
                                                         : 0);
  }
