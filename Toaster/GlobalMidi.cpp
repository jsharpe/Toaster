...
void GlobalMidi::midiApplyOperationMode(unsigned short rawVal) {
  if (rawVal == 1) {
    ...
    Midi::get().sendCmd(cc);
    Midi::get().sendCmd(
        createSingleParamSetCmd(getAddressPage(), sOperationMode, rawVal));
  }
}

void GlobalMidi::midiApplyConnectName(const QString &connectName) {
  Midi::get().sendCmd(
      createStringParamSetCmd(getAddressPage(), sConnectName, connectName));
}
...
void GlobalMidi::midiSendBeacon() {
  ...
  value.push_back(
      0x6f); // todo: check ack  (fct==0x7e, ap==0x7f, param==counter)
...
ByteArray GlobalMidi::getAddressPage() { return sAddressPage; }
