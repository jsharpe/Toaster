...
Stomp::Stomp(StompInstance instance)
    : QObject(0), StompMidi(instance), mFXType(None) {}
...
void Stomp::applyCompressorGateIntensity(double compressorGateIntensity) {
  midiApplyCompressorGateIntensity(
      Utils::phys2Raw(compressorGateIntensity, 10, 0));
}
...
void Stomp::applyModulationHyperChorusAmount(
    double modulationHyperChorusAmount) {
  midiApplyModulationHyperChorusAmount(
      Utils::phys2Raw(modulationHyperChorusAmount, 2, 1));
}
...
void Stomp::applyModulationPhaserPeakSpread(double modulationPhaserPeakSpread) {
  midiApplyModulationPhaserPeakSpread(
      Utils::phys2Raw(modulationPhaserPeakSpread, 10, -5));
}

void Stomp::applyModulationPhaserStages(double modulationPhaserStages) {
  midiApplyModulationPhaserStages((unsigned short)(modulationPhaserStages - 2) /
                                  2);
}
...
void Stomp::applyLowCut(int lowCut) { midiApplyLowCut((unsigned short)lowCut); }

void Stomp::applyHighCut(int highCut) {
  midiApplyHighCut((unsigned short)highCut);
}

void Stomp::applyDelayMix(int mix) { midiApplyDelayMix((unsigned short)mix); }
...
void Stomp::midiDetuneReceived(unsigned short rawVal) {
  emit detuneReceived(Utils::raw2Phys(rawVal, 10, 0));
