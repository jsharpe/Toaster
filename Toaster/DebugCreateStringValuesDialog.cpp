...
DebugCreateStringValuesDialog::DebugCreateStringValuesDialog(QWidget *parent)
    : QDialog(parent), mAdressPage(0), mParameter(0), mLowerRange(0),
      mUpperRange(0x3FFF), mCurrValue(0) {
  ui.setupUi(this);
...
void DebugCreateStringValuesDialog::on_startButton_clicked() {
  if (!ui.apEdit->text().isEmpty() && !ui.paramEdit->text().isEmpty()) {
    ...
    if (mLowerRange <= mUpperRange && !ui.raw2valFileEdit->text().isEmpty() &&
        !ui.val2rawFileEdit->text().isEmpty()) {
...
void DebugCreateStringValuesDialog::onTimer() {
  if (mCurrValue <= mUpperRange) {
    DebugMidi::get().debugRequestStringParam(mAdressPage, mParameter,
                                             mCurrValue);
    ui.progressBar->setValue(mCurrValue);
    mCurrValue++;
  } else if (mTimer) {
