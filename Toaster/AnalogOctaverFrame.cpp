...
AnalogOctaverFrame::AnalogOctaverFrame(QWidget *parent) : QWidget(parent) {
  ui.setupUi(this);
...
void AnalogOctaverFrame::activate(QObject &stomp) {
  ...
  if (mpStomp != nullptr) {
    connect(mpStomp, &Stomp::volumeReceived, this,
            &AnalogOctaverFrame::onVolume);
    connect(mpStomp, &Stomp::duckingReceived, this,
            &AnalogOctaverFrame::onDucking);
    connect(mpStomp, &Stomp::voiceMixReceived, this,
            &AnalogOctaverFrame::onVoiceMix);
    connect(mpStomp, &Stomp::intensityReceived, this,
            &AnalogOctaverFrame::onMix);
    connect(mpStomp,
            static_cast<void (Stomp::*)(double)>(
                &Stomp::modulationCrossoverReceived),
            this, &AnalogOctaverFrame::onLowCut);
    connect(mpStomp, &Stomp::stereoReceived, this,
            &AnalogOctaverFrame::onStereo);
...
void AnalogOctaverFrame::displayStompType(StompInstance stompInstance,
                                          FXType fxType) {
  ui.lcdDisplay->setStompFXType(stompInstance, fxType);
}

void AnalogOctaverFrame::displayStompEnabled(StompInstance stompInstance,
                                             bool enabled) {
