...
ReverbFrame::ReverbFrame(QWidget *parent) : QWidget(parent) {
  ui.setupUi(this);
}
...
void ReverbFrame::activate(QObject &stomp) {
  ...
  if (mpReverb != nullptr) {
    ...
    connect(mpReverb, &Reverb::bandwidthReceived, this,
            &ReverbFrame::onBandwidth);
    connect(mpReverb, &Reverb::centerFrequencyReceived, this,
            &ReverbFrame::onFrequency);
    connect(mpReverb, &Reverb::duckingReceived, this, &ReverbFrame::onDucking);
    connect(mpReverb, &Reverb::delRevBalanceReceived, this,
            &ReverbFrame::onDelRevBalance);
    ...
    ui.lcdDisplay->setStompName(
        LookUpTables::reverbTypeName(mpReverb->getReverbType()));
  }
}

void ReverbFrame::deactivate() {
  if (mpReverb != nullptr) {
    disconnect(mpReverb, &Reverb::preDelayReceived, this,
               &ReverbFrame::onPredaly);
    disconnect(mpReverb, &Reverb::bandwidthReceived, this,
               &ReverbFrame::onBandwidth);
    disconnect(mpReverb, &Reverb::centerFrequencyReceived, this,
               &ReverbFrame::onFrequency);
    disconnect(mpReverb, &Reverb::duckingReceived, this,
               &ReverbFrame::onDucking);
    disconnect(mpReverb, &Reverb::delRevBalanceReceived, this,
               &ReverbFrame::onDelRevBalance);
    disconnect(mpReverb, &Reverb::dampingReceived, this,
               &ReverbFrame::onDamping);
...
void ReverbFrame::displayStompEnabled(StompInstance stompInstance,
                                      bool enabled) {
