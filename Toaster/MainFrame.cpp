...
MainFrame::MainFrame(QWidget *parent)
  ...
  // rig
  // connect(&rigObj, SIGNAL(tempoReceived(double)), this,
  // SLOT(onRigTempo(double)));
  connect(&rigObj, &Rig::volumeReceived, this, &MainFrame::onRigVolume);
  // connect(&rigObj, SIGNAL(tempoEnableReceived(bool)), this,
  // SLOT(onRigTempoEnable(bool)));
  connect(&rigObj, &Rig::stompsEnableReceived, this,
          &MainFrame::onRigStompsEnable);
  connect(&rigObj, &Rig::stackEnableReceived, this,
          &MainFrame::onRigStackEnable);
  connect(&rigObj, &Rig::effectsEnableReceived, this,
          &MainFrame::onRigEffectsEnable);
  // global
  connect(&globalObj, &Global::operationModeReceived, this,
          &MainFrame::onGlobalOperationMode);
  connect(&globalObj, &Global::mainOutputVolumeReceived, this,
          &MainFrame::onGlobalMainVolume);
  connect(&globalObj, &Global::headphoneOutputVolumeReceived, this,
          &MainFrame::onGlobalHeadphoneVolume);
  connect(&globalObj, &Global::monitorOutputVolumeReceived, this,
          &MainFrame::onGlobalMonitorVolume);
  connect(&globalObj, &Global::directOutputVolumeReceived, this,
          &MainFrame::onGlobalDirectVolume);
  // input
  connect(&inputObj, &Input::noiseGateReceived, this,
          &MainFrame::onInputNoiseGate);
  connect(&inputObj, &Input::distortionSenseReceived, this,
          &MainFrame::onInputDistortionSense);
  connect(&inputObj, &Input::cleanSenseReceived, this,
          &MainFrame::onInputCleanSense);

  connect(ui->stompEditor, &StompEditorFrame::editorPageChanged, this,
          &MainFrame::onEditorPageChanged);
  ...
  connect(&mv, &MasterVolume::masterVolumeChanged, [=](int value) {
    ui->masterVolumeDial->setValue(Utils::raw2Phys(value, 10.0, 0));
  });

  connect(&mv, &MasterVolume::linksChanged, [=](int noOfLinks) {
    ui->masterVolumeWidget->setCurrentIndex(noOfLinks == 1 ? 0 : 1);
  });

  connect(ui->headphoneVolumeDial,
          static_cast<void (QToasterLookUpTableDial::*)(int)>(
              &QToasterLookUpTableDial::valueChanged),
          &globalObj, &Global::applyHeadphoneOutputVolume);
  connect(ui->monitorVolumeDial,
          static_cast<void (QToasterLookUpTableDial::*)(int)>(
              &QToasterLookUpTableDial::valueChanged),
          &globalObj, &Global::applyMonitorOutputVolume);

  connect(&profileObj, &Profile::rigNameReceived, this,
          &MainFrame::onRigNameReveived);
}

MainFrame::~MainFrame() { MasterVolume::get().dispose(); }
//------------------------------------------------------------------------------------------
...
void MainFrame::onStompAOnOff(bool onOff) {
  if (mOperationMode != StompEdit) {
    QToasterButton::State state =
        onOff ? QToasterButton::On : QToasterButton::Off;
...
void MainFrame::onModRate(int value) { ui->modRateDial->setValue(value); }
...
void MainFrame::onReverbMix(int mix) { ui->reverbMixDial->setValue(mix); }
//------------------------------------------------------------------------------------------
...
void MainFrame::on_stompsButton_clicked(QToasterButton &bt,
                                        bool /*longClick*/) {
...
void MainFrame::on_effectsButton_clicked(QToasterButton &bt,
                                         bool /*longClick*/) {
...
void MainFrame::onRigStompsEnable(bool stompsEnable) {
  QToasterButton::State state =
      stompsEnable ? QToasterButton::On : QToasterButton::Off;
...
void MainFrame::onRigStackEnable(bool stackEnable) {
  QToasterButton::State state =
      stackEnable ? QToasterButton::On : QToasterButton::Off;
...
void MainFrame::onRigEffectsEnable(bool effectsEnable) {
  QToasterButton::State state =
      effectsEnable ? QToasterButton::On : QToasterButton::Off;
...
// global
// ui => kpa
void MainFrame::on_chickenHeadDial_valueChanged(
    const QChickenHeadDial::State &state) {
...
void MainFrame::onGlobalMainVolume(int volume) { update(); }
...
void MainFrame::onGlobalDirectVolume(double volume) { update(); }
//------------------------------------------------------------------------------------------
...
// utility methods
void MainFrame::handleStompButtonClick(QObject &module, QToasterButton &stompBt,
                                       bool longClick) {
  if (longClick) {
...
void MainFrame::toggleOperationMode(QObject &module, OperationMode opMode,
                                    QToasterButton *bt) {
  if (bt != nullptr) {
    ...
    if (bt->state() == QToasterButton::Blinking) {
      ...
      ui->modeFrames->setCurrentIndex(
          ui->modeFrames->indexOf(ui->browser)); // todo:
...
void MainFrame::on_exitButton_clicked(QToasterButton & /*bt*/,
                                      bool /*longClick*/) {
  if (mOperationMode == StompEdit && mEditModeButton != nullptr &&
      mEditModeModule != nullptr) {
    toggleOperationMode(*mEditModeModule, mPreviousOperationMode,
                        mEditModeButton);
  }
}

void MainFrame::on_prevPageButton_clicked(QToasterButton & /*bt*/,
                                          bool /*longClick*/) {
  ui->stompEditor->prevDisplayPage();
}

void MainFrame::on_nextPageButton_clicked(QToasterButton & /*bt*/,
                                          bool /*longClick*/) {
...
void MainFrame::onEditorPageChanged(IStompEditorPage *editorPage) {
  if (editorPage == nullptr ||
      editorPage->getMaxDisplayPage() == QToasterLCD::Page1) {
...
void MainFrame::on_outputButton_clicked(QToasterButton &bt,
                                        bool /*longClick*/) {
