...
CompressorFrame::CompressorFrame(QWidget *parent) : QWidget(parent) {
  ui.setupUi(this);
}
...
void CompressorFrame::activate(QObject &stomp) {
  ...
  if (mpStomp != nullptr) {
    connect(mpStomp, &Stomp::compressorGateIntensityReceived, this,
            &CompressorFrame::onIntensity);
    connect(mpStomp, &Stomp::compressorAttackReceived, this,
            &CompressorFrame::onAttack);
    connect(mpStomp, &Stomp::compressorSquashReceived, this,
            &CompressorFrame::onSquash);
...
void CompressorFrame::deactivate() {
  if (mpStomp != nullptr) {
    disconnect(mpStomp, &Stomp::compressorGateIntensityReceived, this,
               &CompressorFrame::onIntensity);
    disconnect(mpStomp, &Stomp::compressorAttackReceived, this,
               &CompressorFrame::onAttack);
    disconnect(mpStomp, &Stomp::compressorSquashReceived, this,
               &CompressorFrame::onSquash);
    disconnect(mpStomp, &Stomp::volumeReceived, this,
               &CompressorFrame::onVolume);
...
void CompressorFrame::displayStompType(StompInstance stompInstance,
                                       FXType fxType) {
  ui.lcdDisplay->setStompFXType(stompInstance, fxType);
}

void CompressorFrame::displayStompEnabled(StompInstance stompInstance,
                                          bool enabled) {
