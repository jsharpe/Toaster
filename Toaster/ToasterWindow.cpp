...
ToasterWindow::ToasterWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::ToasterWindow), mIsConnected2Midi(false),
      mIsConnected2KPA(false), mDataReceivedfromKPA(false) {
  ui->setupUi(this);
  ui->statusBar->addPermanentWidget(&mConnectionStatus);

  QString title =
      "Toaster " + QString(APP_VERSION) + QString(" ") + QString(APP_STAGE);
  setWindowTitle(title);

  connect(this, &ToasterWindow::connectionStatusChanged,
          [=](bool midiConnected, bool, bool) {
            ui->actionUploadKIPRFile->setEnabled(midiConnected);
          });
  connect(this, &ToasterWindow::connectionStatusChanged,
          [=](bool midiConnected, bool, bool) {
            ui->actionClose_MIDI_Ports->setEnabled(midiConnected);
          });
  connect(this, &ToasterWindow::connectionStatusChanged,
          [=](bool midiConnected, bool, bool) {
            ui->actionRequestValues->setEnabled(midiConnected);
          });
  connect(this, &ToasterWindow::connectionStatusChanged,
          [=](bool midiConnected, bool, bool) {
            ui->actionOpenMIDIPorts->setDisabled(midiConnected);
          });
  connect(this, &ToasterWindow::connectionStatusChanged,
          [=](bool midiConnected, bool kpaConnected, bool) {
            ui->actionDisconnectFromKPA->setEnabled(midiConnected &&
                                                    kpaConnected);
          });
  connect(this, &ToasterWindow::connectionStatusChanged,
          [=](bool midiConnected, bool kpaConnected, bool) {
            ui->actionConnectToKPA->setEnabled(midiConnected && !kpaConnected);
          });
  connect(this, &ToasterWindow::connectionStatusChanged,
          [=](bool midiConnected, bool, bool) {
            mConnectionStatus.setMidiStatus(midiConnected);
          });
  connect(this, &ToasterWindow::connectionStatusChanged,
          [=](bool midiConnected, bool kpaConnected, bool dataReceived) {
            mConnectionStatus.setKPAStatus(midiConnected && kpaConnected,
                                           dataReceived);
          });
...
void ToasterWindow::on_actionConfigure_triggered() { showSettingsDialog(); }
...
void ToasterWindow::on_actionUploadKIPRFile_triggered() {
  ...
  // if(magic == 0x6468544d || magic == 0x6468544b)
  {
    ...
    // if(dataBytesInFile == kiprFile.bytesAvailable())
    {
      ...
      ByteArray c1 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f, 0x01,
                      0x00, 0x7f, 0x7c, 0x00, 0x00, 0xf7};
      ByteArray c2 = {0xf0, 0x00, 0x20, 0x33, 0x02,
                      0x7f, 0x03, 0x00, 0x7f, 0x7d};
      QString appName("Toaster");
      for (int i = 0; i < appName.length(); ++i)
        c2.push_back(appName.at(i).toLatin1());
      c2.push_back(0x00);
      c2.push_back(0xf7);
      ByteArray c3 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f, 0x03, 0x00,
                      0x7f, 0x7c, 0x52, 0x65, 0x63, 0x65, 0x69, 0x76,
                      0x69, 0x6e, 0x67, 0x20, 0x52, 0x69, 0x67, 0x73,
                      0x2e, 0x2e, 0x2e, 0x00, 0xf7};
      ByteArray c4 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f, 0x03, 0x00, 0x7f,
                      0x7c, 0x52, 0x65, 0x63, 0x65, 0x69, 0x76, 0x69, 0x6e,
                      0x67, 0x20, 0x69, 0x74, 0x65, 0x6d, 0x20, 0x31, 0x20,
                      0x6f, 0x66, 0x20, 0x31, 0x00, 0xf7};
      ByteArray c5 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f,
                      0x03, 0x00, 0x7f, 0x7b, 0x00, 0xf7};
      ...
      ByteArray r1 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f, 0x7e,
                      0x00, 0x03, 0x01, 0x00, 0x00, 0x4e, 0xf7};
      {
        Midi::get().sendCmd(r1);
        QString fileName = QFileDialog::getOpenFileName(
            this, tr("Open File"),
      ...
      ByteArray s2 = {0xf0, 0x00, 0x20, 0x33, 0x02,
                      0x7f, 0x7e, 0x00, 0x06, 0x01};
      ...
      while (loop) {
        ...
        ByteArray r2 = {0xf0, 0x00, 0x20, 0x33, 0x02,
                        0x7f, 0x7e, 0x00, 0x04, 0x01};
      ...
      ByteArray cmd1 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f,
                        0x7e, 0x00, 0x02, 0x01, 0x01, 0x00};
      ByteArray cmd2 = {0xf0, 0x00, 0x20, 0x33, 0x02,
                        0x7f, 0x7e, 0x00, 0x08, 0x01};
      ...
      ByteArray e1(c1);
      ByteArray e2(c2);
      ByteArray e3 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f, 0x03, 0x00, 0x7f,
                      0x7c, 0x52, 0x65, 0x63, 0x65, 0x69, 0x76, 0x69, 0x6e,
                      0x67, 0x20, 0x69, 0x74, 0x65, 0x6d, 0x20, 0x31, 0x20,
                      0x6f, 0x66, 0x20, 0x31, 0x00, 0xf7};
      ByteArray e4 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f,
                      0x03, 0x00, 0x7f, 0x7b, 0x00, 0xf7};
      ByteArray e5 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f, 0x01,
                      0x00, 0x7f, 0x7c, 0x00, 0x64, 0xf7};
      ByteArray e6 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f, 0x01,
                      0x00, 0x7f, 0x7c, 0x00, 0x00, 0xf7};
      ByteArray e7 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f,
                      0x03, 0x00, 0x7f, 0x7d, 0x00, 0xf7};
      ByteArray e8 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f,
                      0x03, 0x00, 0x7f, 0x7c, 0x00, 0xf7};
      ByteArray e9 = {0xf0, 0x00, 0x20, 0x33, 0x02, 0x7f,
                      0x03, 0x00, 0x7f, 0x7b, 0x00, 0xf7};
...
void ToasterWindow::on_actionOpenMIDIPorts_triggered() { openMidiPorts(); }
...
void ToasterWindow::on_actionExit_triggered() { QApplication::quit(); }
...
void ToasterWindow::on_actionSendSySex_triggered() {
  ...
  QByteArray value =
  {0x01,0x4F,0x41,0x2D,0x47,0x69,0x6C,0x6D,0x6F,0x75,0x72,0x20,0x2D,0x20,0x32,0x30,0x31,0x36,0x2D,0x30,0x39,0x2D,0x30,0x36,0x20,0x31,0x38,0x2D,0x32,0x31,0x2D,0x31,0x39,0x2E,0x6B,0x69,0x70,0x72,0x00};
