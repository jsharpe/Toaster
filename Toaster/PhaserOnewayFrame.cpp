...
PhaserOnewayFrame::PhaserOnewayFrame(QWidget *parent) : QWidget(parent) {
  ui.setupUi(this);
}
...
void PhaserOnewayFrame::activate(QObject &stomp) {
  ...
  if (mpStomp != nullptr) {
    connect(
        mpStomp,
        static_cast<void (Stomp::*)(double)>(&Stomp::modulationRateReceived),
        this, &PhaserOnewayFrame::onRate);
    connect(mpStomp, &Stomp::modulationDepthReceived, this,
            &PhaserOnewayFrame::onDepth);
    connect(mpStomp, &Stomp::modulationManualReceived, this,
            &PhaserOnewayFrame::onManual);
    connect(mpStomp, &Stomp::modulationFeedbackReceived, this,
            &PhaserOnewayFrame::onFeedback);
    connect(mpStomp, &Stomp::modulationPhaserPeakSpreadReceived, this,
            &PhaserOnewayFrame::onPeakSpread);
    connect(mpStomp, &Stomp::modulationPhaserStagesReceived, this,
            &PhaserOnewayFrame::onStages);
    connect(mpStomp, &Stomp::mixReceived, this, &PhaserOnewayFrame::onMix);
    connect(mpStomp, &Stomp::duckingReceived, this,
            &PhaserOnewayFrame::onDucking);
    connect(mpStomp, &Stomp::volumeReceived, this,
            &PhaserOnewayFrame::onVolume);
    connect(mpStomp, &Stomp::stereoReceived, this,
            &PhaserOnewayFrame::onStereo);
    ...
    if (mpStomp->getInstance() != StompX &&
        mpStomp->getInstance() != StompMod) {
...
void PhaserOnewayFrame::deactivate() {
  if (mpStomp != nullptr) {
    disconnect(
        ...
        static_cast<void (Stomp::*)(double)>(&Stomp::modulationRateReceived),
        this, &PhaserOnewayFrame::onRate);
    disconnect(mpStomp, &Stomp::modulationDepthReceived, this,
               &PhaserOnewayFrame::onDepth);
    disconnect(mpStomp, &Stomp::modulationManualReceived, this,
               &PhaserOnewayFrame::onManual);
    disconnect(mpStomp, &Stomp::modulationFeedbackReceived, this,
               &PhaserOnewayFrame::onFeedback);
    disconnect(mpStomp, &Stomp::modulationPhaserPeakSpreadReceived, this,
               &PhaserOnewayFrame::onPeakSpread);
    disconnect(mpStomp, &Stomp::modulationPhaserStagesReceived, this,
               &PhaserOnewayFrame::onStages);
...
void PhaserOnewayFrame::displayStompType(StompInstance stompInstance,
                                         FXType fxType) {
  ui.lcdDisplay->setStompFXType(stompInstance, fxType);
}

void PhaserOnewayFrame::displayStompEnabled(StompInstance stompInstance,
                                            bool enabled) {
